...
def main():
    ...
    # Constants
    CONFIG_FILE_PATH = '/app/data/conf.json'  # Standardized config file name inside the container
    ...
    # MQTT parameters with sensible defaults
    MQTT_HOST = config_data.get("MQTT_HOST", "localhost")
    MQTT_PORT = int(config_data.get("MQTT_PORT", 1883))
    MQTT_ENABLE_SSL = config_data.get("MQTT_ENABLE_SSL", False)
    MQTT_USER = config_data.get("MQTT_USER", "")
    MQTT_PASSWORD = config_data.get("MQTT_PASSWORD", "")
    SUBSCRIBER_NAME = config_data.get("SUBSCRIBER_NAME", "default_subscriber")
    SUBSCRIBER_DESCRIPTION = config_data.get("SUBSCRIBER_DESCRIPTION", "Default MQTT Subscriber Description")

    # Read topic components into a list, assuming 'TOPICS' is an array of strings
    topic_components = config_data.get("TOPICS", [])

    # Create the MQTT topic by joining the components with '/'
    # This will handle any number of topic components provided in the configuration
