...
class MQTTClient:
    def __init__(self,
        ...
        if self.mqtt_enable_ssl:
            self.client.tls_set()  # Add arguments if necessary (certs, keys, etc.)
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            self.flag_connected = False
            logging.error('Failed to connect to MQTT host: %s on port: %s, result code: %s', 
                          self.mqtt_host, self.mqtt_port, rc)

    def on_disconnect(self, client, userdata, rc):
        """ Callback on disconnect """
        self.flag_connected = False
        logging.info('Disconnected from MQTT host: %s on port: %s with result code: %s', 
                     self.mqtt_host, self.mqtt_port, rc)
        client.loop_stop()
    ...
    def subscribe(self, topic, qos=0):
        ...
        if self.flag_connected:
            ...
            logging.error("""Cannot subscribe because the client is
                           not connected.""")
